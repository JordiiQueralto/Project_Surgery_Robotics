...
// Receive torques from Servomotors module 
// and control motor accordingly 
...
void receiveTorquesUDP() {
  ...
  if (packetSize) {
    ...
    if (!error) {
      ...
      // Filter packets that come from Servomotors module
      if (device && strcmp(device, "Servomotors") == 0) {
        Torque_roll1 = doc["Torque_roll1"] | 0.0;
        Torque_pitch = doc["Torque_pitch"] | 0.0;
        Torque_yaw   = doc["Torque_yaw"]   | 0.0;

        // --- Control del motor de vibració ---
        float totalTorque = Torque_roll1 + Torque_pitch + Torque_yaw;
        int vibrationValue = constrain(totalTorque * 2.5, 0, 255); // escala 2.5 segons enunciat
        ledcWrite(0, vibrationValue); // escriu PWM al canal 0
...
void loop() {
  updateOrientation(); // Actualitza angles del gripper
  sendOrientationUDP(); // Envia orientació a la resta del sistema
  receiveTorquesUDP(); // Rep torques i controla la vibració 
