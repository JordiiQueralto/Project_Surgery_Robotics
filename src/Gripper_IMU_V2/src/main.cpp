...
float Torque_roll1= 0.0; float Torque_roll2= 0.0;float Torque_pitch = 0.0; float Torque_yaw = 0.0;
...
void receiveTorquesUDP() {
  ...
  if (packetSize) {
    ...
    if (len > 0) {
      packetBuffer[len] = '\0';
      Serial.print("Received torque packet: ");
      ...
      if (strcmp(device, "G3_Servos") == 0) {
        ...
        ledcWrite(0, vibrationValue); // PWM output to vibration motor
...
void setup() {
  ...
  ledcWrite(0, 0); // Start with vibration off
}
